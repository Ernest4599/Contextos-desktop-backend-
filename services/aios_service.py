...
from services.models import AiosMemory, AiosEntity, AiosRelationship
...
6. Separately, identify any ENTITIES worth remembering as part of the Brain graph - people, organizations, products, platforms, projects, or concepts the user mentioned. For each entity:
...
def tell_aios(db: Session, user_id: int, raw_input: str, allowed_providers: list[str] | None = None) -> Dict[str, Any]:
    ...
    # --- Resolve entities and relationships (AIOS Brain graph) ---
...
def get_brain_graph(db: Session, user_id: int) -> Dict[str, Any]:
    """
